...
public void insert_counterparty(String name, String address, String tax_number,String phone_number) throws SQLException, ClassNotFoundException {
    ...
    public void delete_counetrparty(String tax_number) throws SQLException, ClassNotFoundException
    {
        Connection conn=get_database_connection();
        Statement stm;
        stm = conn.createStatement();
        String sql = "delete from counterparty where tax_number=?";
        PreparedStatement pstmt = conn.prepareStatement(sql);
        pstmt.setString(1, tax_number);
        pstmt.executeUpdate();
    }
